...
func (pm *PermissionManager) CreateCustomRole(ctx context.Context, userID, orgID, role string, permissions ...string) *apperror.AppError {
...
func (pm *PermissionManager) DeleteCustomRole(ctx context.Context, userID, orgID, role string) *apperror.AppError {
	roleObj := GenerateCustomRoleObject(orgID, role)
	return pm.tupleManager.Delete(ctx, []client.ClientTupleKeyWithoutCondition{
		{
			User:     "organization:" + orgID,
