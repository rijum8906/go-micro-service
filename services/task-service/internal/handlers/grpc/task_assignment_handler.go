...
func (h *TaskHandler) AssignTask(ctx context.Context, req *taskv1.AssignTaskRequest) (*modelsv1.TaskAssignment, error) {
	...
	if !ok {
		return nil, utils.MapAppError(apperror.ErrUnAuthenticated.WithMessage("assign task user metadata is required"))
	}
...
func (h *TaskHandler) UnassignTask(ctx context.Context, req *taskv1.UnassignTaskRequest) (*corev1.SuccessResponse, error) {
	...
	userInfo, ok := metadata.ReceiveUserInfo(ctx)
	if !ok {
		return nil, utils.MapAppError(apperror.ErrUnAuthenticated.WithMessage("unassign task user metadata is required"))
	}

	result, appErr := h.taskAssignmentService.UnassignTask(ctx, req, &userInfo)
	if appErr != nil {
		return nil, utils.MapAppError(appErr)
	}
...
func (h *TaskHandler) ReassignTask(ctx context.Context, req *taskv1.ReassignTaskRequest) (*modelsv1.TaskAssignment, error) {
	...
	userInfo, ok := metadata.ReceiveUserInfo(ctx)
	if !ok {
		return nil, utils.MapAppError(apperror.ErrUnAuthenticated.WithMessage("reassign task user metadata is required"))
	}
...
func (h *TaskHandler) ListTaskAssignments(ctx context.Context, req *taskv1.ListTaskAssignmentsRequest) (*taskv1.ListTaskAssignmentsResponse, error) {
	...
	userInfo, ok := metadata.ReceiveUserInfo(ctx)
	if !ok {
		return nil, utils.MapAppError(apperror.ErrUnAuthenticated.WithMessage("list task assignments user metadata is required"))
	}

	result, appErr := h.taskAssignmentService.ListTaskAssignments(ctx, req, &userInfo)
	if appErr != nil {
		return nil, utils.MapAppError(appErr)
	}
