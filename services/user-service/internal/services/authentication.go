// Package services contains business logic for the user service.
package services
...
func (s *authService) Signin(ctx context.Context, data dto.SigninRequest, reqMetadata dto.RequestMetadata) (*dto.AuthResponse, *appError.AppError) {
	...
	if err := s.createSession(ctx, account.ID, refreshToken, reqMetadata); err != nil {
		return nil, err
	}

	accessToken, issueErr := s.utilsConfig.JwtService.IssueToken(ctx, FormatUUID(account.ID))
	if issueErr != nil {
		return nil, appError.ErrInternal.WithInternal(issueErr)
	}
...
func (s *authService) ResetPassword(ctx context.Context, data dto.ResetPasswordRequest, reqMetadata dto.RequestMetadata) *appError.AppError {
	...
	_, err = s.q.UpdateAccount(ctx, db.UpdateAccountParams{ID: pgUUID, PasswordHash: pgtype.Text{String: hashPass, Valid: true}})
	if err != nil {
		return appError.ErrInternal.WithInternal(err)
	}
...
func (s *authService) createSession(ctx context.Context, accountID pgtype.UUID, refreshToken string, reqMetadata dto.RequestMetadata) *appError.AppError {
