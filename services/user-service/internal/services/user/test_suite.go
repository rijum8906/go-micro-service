...
func (suite *TestSuite) TearDownSuite() {
	...
	suite.UserService.DBPool.Close()      // Close the database pool
}

func (suite *TestSuite) CreateUser(t *testing.T, email, password string) (*db.User, *db.Profile, error) {
	passwordHash, appErr := suite.AuthService.HashService.Hash(password)
	if appErr != nil {
		return nil, nil, appErr
	}
	user, err := suite.UserService.DBQ.CreateUser(suite.Ctx, db.CreateUserParams{
		Email: email,
	...
	t.Cleanup(func() {
		suite.UserService.DBQ.DeleteUserHard(context.Background(), user.ID)
	})

	profile, err := suite.UserService.DBQ.CreateProfile(suite.Ctx, db.CreateProfileParams{
		UserID: user.ID,
	})
	if err != nil {
		return nil, nil, err
	...
	t.Cleanup(func() {
		suite.UserService.DBQ.DeleteProfileHard(context.Background(), profile.ID)
	})
