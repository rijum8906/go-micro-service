...
func NewTestSuite() *TestSuite {
	return &TestSuite{
		AuthService: NewForTest(),
		Ctx:         context.Background(),
	}
}

func (suite *TestSuite) SetupSuite() {
	suite.AuthService = NewForTest()
...
func (suite *TestSuite) CreateUser(t *testing.T, email, password string) (*db.User, *db.Profile, error) {
	...
	user, err := suite.AuthService.DBQ.CreateUser(suite.Ctx, db.CreateUserParams{
		Email: email,
	...
	profile, err := suite.AuthService.DBQ.CreateProfile(suite.Ctx, db.CreateProfileParams{
		UserID: user.ID,
	})
	if err != nil {
		return nil, nil, err
	...
	t.Cleanup(func() {
		suite.AuthService.DBQ.DeleteProfileHard(context.Background(), profile.ID)
	})
...
func (suite *TestSuite) SetUserInfoInContext(user *db.User) {
