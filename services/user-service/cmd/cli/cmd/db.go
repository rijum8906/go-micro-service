...
var dbCmd = &cobra.Command{
	...
	Long:  "Manage Atlas-based and embedded SQL migrations for the user service database.",
}
...
func newDBSQLApplyCmd() *cobra.Command {
	return &cobra.Command{
		Use:     "apply",
		Aliases: []string{"migrate"},
		Short:   "Apply embedded SQL migrations without Atlas",
		RunE: func(cmd *cobra.Command, args []string) error {
			fmt.Println("Applying embedded SQL migrations...")
			if err := applyEmbeddedMigrations(dbURL()); err != nil {
...
func init() {
	...
	dbSQLCmd.AddCommand(newDBSQLApplyCmd())

	dbCmd.AddCommand(dbAtlasCmd)
	dbCmd.AddCommand(dbSQLCmd)

	// Keep the Atlas commands at the top level as compatibility aliases for existing make targets.
